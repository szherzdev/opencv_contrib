...
#include "modules/ximgproc/opencl_kernels_ximgproc.hpp"
...
void DTFilterOCL::filterRF_naiveAlg(UMat& res, UMat& resT, UMat& adistHor, UMat& adistVert)
{
    ...
    size_t localSize[] = { std::min( (size_t)32, ocl::Device::getDefault().maxWorkGroupSize() ) };
    ...
    for (int iter = 1; iter <= numIters; iter++)
    {
        ...
        MEASURE_KER_TIME(statusHorPass = kerHor.run(1, globSizeHor, localSize, syncQueue));
        ...
        MEASURE_KER_TIME(statusVertPass = kerVert.run(1, globSizeVert, localSize, syncQueue));
